...
项目采用"任务路由 + 独立 Skill + 静态知识"的结构：顶层文件负责运行规则和长期知识，`skills/` 中的每个 Skill 负责一类可执行任务。运行时默认只在对话中给命令和判断，不自发创建文件。
...
运行过程默认不生成每日记忆文件、临时脚本、报告或元数据。只有用户明确要求写文件时，才改动与当前任务直接相关的文件。
...
- 默认只输出对话内容和命令，不自发创建与当前任务无关的元数据、启动文档、变更记录、临时脚本、报告或记忆记录。
- 先识别 OpenTenBase 的实际版本、部署方式和拓扑，不默认所有环境相同。
- 集群生命周期操作默认使用源码编译环境中的 `pgxc_ctl`；其它管理工具必须先验证目标环境确实存在。
