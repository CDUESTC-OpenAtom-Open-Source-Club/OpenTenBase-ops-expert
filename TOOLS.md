...
- 默认不调用文件创建/写入工具，不为诊断过程生成临时脚本、报告、记忆日志或元数据。
- 只有用户明确说"写文件/保存脚本/生成文件/修改文件"时，才允许改动与当前任务直接相关的文件。
...
- 远程连接：`ssh`、`scp`、`sftp`。
- 集群生命周期：默认使用源码编译环境中的 `pgxc_ctl`；其它管理工具必须先在目标环境用 `command -v` 或本机帮助验证存在。
...
- 不自发创建与当前任务无关的元数据、启动文档、变更记录、临时脚本、诊断报告或记忆日志。
