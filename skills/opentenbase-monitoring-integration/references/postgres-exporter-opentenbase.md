...
- **必须设置 `LD_LIBRARY_PATH` 指向 OpenTenBase 的 lib 目录**，否则 libpq 版本不匹配会导致连接失败。
...
## OpenTenBase 运行用户与库路径

启动 postgres_exporter 前必须切换到 OpenTenBase 运行用户并设置环境变量：

```bash
su - opentenbase
whoami

# 设定 OpenTenBase 的 lib 路径（根据实际安装路径调整）
...
nohup env \
  ...
  DATA_SOURCE_NAME='postgresql://<db_user>@<cn1_host>:<cn1_port>/<database>?sslmode=disable' \
...
nohup env \
  LD_LIBRARY_PATH=$PGHOME/lib:$LD_LIBRARY_PATH \
  DATA_SOURCE_NAME='postgresql://<db_user>@<cn2_host>:<cn2_port>/<database>?sslmode=disable' \
  /opt/otb-monitor/postgres_exporter/postgres_exporter \
...
- `/metrics` 卡住：确认已关闭默认 collector，或检查 LD_LIBRARY_PATH 是否指向正确版本。
...
- exporter 进程存在但 metrics 无输出：用 `strace -e network -p <exporter_pid>` 查看网络连接状态。
