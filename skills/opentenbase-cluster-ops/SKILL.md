...
不要在 root 环境下直接判断 `pgxc_ctl`、`psql` 是否可用。OpenTenBase 工具通常依赖运行用户的 `PATH`、`LD_LIBRARY_PATH`、工作目录和文件权限。
...
OpenTenBase 有两种主流管理方式，部署方式决定用哪个：

| 部署方式 | 管理工具 | 典型安装路径 |
|---------|---------|------------|
| 源码编译 + `pgxc_ctl` | `pgxc_ctl` | `/data/opentenbase/pgxc_ctl/` |
| 一键脚本 (`opentenbase.sh`) | 进程+psql | `/var/lib/opentenbase/install/opentenbase/<version>/` |
| RPM/DEB 包安装 | `opentenbase_ctl` | `/usr/bin/opentenbase_ctl` |

进入运行用户后自动检测：

```bash
# 检测管理工具
if command -v pgxc_ctl 2>/dev/null; then
  echo "MANAGER=pgxc_ctl"
elif command -v opentenbase_ctl 2>/dev/null; then
  echo "MANAGER=opentenbase_ctl"
else
  echo "MANAGER=manual (psql + process check)"
fi

# 检测二进制路径
ls /var/lib/opentenbase/install/opentenbase/ 2>/dev/null && echo "INSTALL=script-deploy"
ls /usr/lib/opentenbase/ 2>/dev/null && echo "INSTALL=rpm-deploy"
ls /data/opentenbase/install/ 2>/dev/null && echo "INSTALL=source-build"
```
...
若目标环境提供了其它管理工具，必须先在运行用户下用 `command -v <tool>` 和 `<tool> --help` 验证存在，再按本机帮助输出命令；不得凭记忆假设工具存在。

切换工具前说明原因、工具、配置和计划命令，并获得用户确认。
...
涉及 `pgxc_ctl` 配置文件、节点 IP、端口或目录变更时，读取：
...
> 用户：看看集群状态
>
> Agent：（通过 SSH 连接服务器，执行 `pgxc_ctl --home /data/opentenbase/pgxc_ctl --configuration /data/opentenbase/pgxc_ctl/pgxc_ctl.conf monitor all`）
> ```
> OpenTenBase：正常
> 目标：192.168.1.10
> 方式：pgxc_ctl
...
> 用户：启动 OpenTenBase
>
> Agent：检测到集群处于停止状态。将执行 `pgxc_ctl --home /data/opentenbase/pgxc_ctl --configuration /data/opentenbase/pgxc_ctl/pgxc_ctl.conf start all`，目标主机 192.168.1.10，运行用户 opentenbase。现在启动？
>
> 用户：启动
>
