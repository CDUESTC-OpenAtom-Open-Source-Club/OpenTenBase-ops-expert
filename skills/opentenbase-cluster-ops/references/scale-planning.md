...
- 当前实验环境中安装版 `opentenbase_ctl --help` 未显示 `expand`、`shrink` 子命令。
...
- 当前用户环境中的 `opentenbase_ctl expand/shrink` 是否可用。
...
```bash
command -v opentenbase_ctl pgxc_ctl psql 2>/dev/null || true
...
opentenbase_ctl expand --help
opentenbase_ctl shrink --help
...
- 是否需要修改 `opentenbase_ctl` 或 `pgxc_ctl` 配置。
...
当前管理工具：opentenbase_ctl / pgxc_ctl / 不确定
