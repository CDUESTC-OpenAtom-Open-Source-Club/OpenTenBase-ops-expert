...
在每个节点的 `postgresql.conf`（或经 `pgxc_ctl` / `opentenbase_ctl` 下发的配置）中：
...
> ⚠️ 修改 `archive_mode` / `wal_level` 并重启节点属于高风险操作，必须先获得用户确认，并在业务低峰逐节点滚动执行。
...
> ⚠️ 以下为**恢复流程框架**，涉及停库、覆盖数据目录、写 recovery 配置，全部为高危操作。Agent 不得自动执行，必须由用户逐步确认。这里给出正确的**顺序和参数**，避免用户遗漏关键步骤。
...
**输出方案时必须一并给出**：备份存储位置与容量、保留策略、归档监控告警（`pg_stat_archiver.failed_count`）、演练计划。只给"跑个 pg_dumpall 定时任务"不算完整策略。
